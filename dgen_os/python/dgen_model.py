...
def main(mode = None, resume_year = None, endyear = None, ReEDS_inputs = None):
    ...
    try:
        ...
        scenario_names = []
        dup_n = 1
        out_subfolders = {'wind': [], 'solar': [], 'storage': []}
        for i, scenario_file in enumerate(model_settings.input_scenarios):
            ...
            if scenario_settings.techs in [['wind'], ['solar'], ['storage']]:
            ...
            if scenario_settings.techs == ['solar']:
                ...
                for i, year in enumerate(scenario_settings.model_years):
                    ...
                    solar_agents.chunk_on_row(financial_functions.calc_system_size_and_performance_solar, sectors=scenario_settings.sectors, cores=cores, rate_switch_table=rate_switch_table)
                    ...
                    solar_agents.on_frame(agent_mutation.elec.estimate_total_generation_solar)
            ...
            elif scenario_settings.techs == ['storage']:
                ...
                for i, year in enumerate(scenario_settings.model_years):
                    ...
                    state_capacity_by_year = agent_mutation.elec.calc_state_capacity_by_year_storage(con, schema, load_growth, peak_demand_mw, is_first_year, year,solar_agents,last_year_installed_capacity)
                    
                    #Apply net metering parameters
                    net_metering_state_df, net_metering_utility_df = agent_mutation.elec.get_nem_settings_storage(nem_state_capacity_limits, nem_state_and_sector_attributes, nem_utility_and_sector_attributes, nem_selected_scenario, year, state_capacity_by_year, cf_during_peak_demand)
                    ...
                    solar_agents.on_frame(agent_mutation.elec.apply_export_tariff_params_storage, [net_metering_state_df, net_metering_utility_df])
                    ...
                    # Apply state incentives
                    solar_agents.on_frame(agent_mutation.elec.apply_state_incentives_storage, [state_incentives, year, model_settings.start_year, state_capacity_by_year])

                    # Calculate System Financial Performance
                    solar_agents.chunk_on_row(financial_functions.calc_system_size_and_performance_storage, sectors=scenario_settings.sectors, cores=cores, rate_switch_table=rate_switch_table)
                    ...
                    if is_first_year == True:
                        state_starting_capacities_df = agent_mutation.elec.get_state_starting_capacities(con, schema)
                        solar_agents.on_frame(agent_mutation.elec.estimate_initial_market_shares_storage, state_starting_capacities_df)
                        market_last_year_df = None
                    else:
                        solar_agents.on_frame(agent_mutation.elec.apply_market_last_year, market_last_year_df)

                    solar_agents.df.to_csv('solar_agents.csv')
                    print('Executed export to CSV')

                    # Calculate diffusion based on economics and bass diffusion
                    solar_agents.df, market_last_year_df = diffusion_functions_elec.calc_diffusion_storage(solar_agents.df, is_first_year, bass_params, year)

                    # Estimate total generation
                    solar_agents.on_frame(agent_mutation.elec.estimate_total_generation_storage)

                    # Aggregate results
                    scenario_settings.output_batt_dispatch_profiles = True

                    last_year_installed_capacity = solar_agents.df[['state_abbr','batt_kw_cum','batt_kwh_cum','year']].copy()
                    last_year_installed_capacity = last_year_installed_capacity.loc[last_year_installed_capacity['year'] == year]
                    last_year_installed_capacity = last_year_installed_capacity.groupby('state_abbr')[['batt_kw_cum','batt_kwh_cum']].sum().reset_index()
